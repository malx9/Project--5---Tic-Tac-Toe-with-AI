...
const humanTurn = () => {
  ...
  const aiTurn = () => {
    ...
    const possibleChoices = matchingPatterns.flat();

    // const indexForChoice = Math.floor(
    //   Math.random() * possibleChoices.length - 1
    // );

    let indexForChoice;
    do {indexForChoice= Math.floor(Math.random() * possibleChoices.length - 1} while (!squaresLeft.includes(possibleChoices[indexForChoice]))
    ...
    if (squaresLeft.includes(aiChoice)) {
