...
const humanTurn = () => {
  ...
  const aiTurn = () => {
    ...
    let possibleChoices = matchingPatterns
      .flat()
      .filter((choice) => !humanChoices.includes(choice));

    // const filteredPossibleChoices = possibleChoices.filter(
    //   (choice) => !humanChoices.includes(choice)
    // );

    console.log("test", possibleChoices);
    console.log("filtered", filteredPossibleChoices);

    let indexForChoice;
    do {
      indexForChoice = Math.floor(
        Math.random() * filteredPossibleChoices.length - 1
      );
    } while (!squaresLeft.includes(filteredPossibleChoices[indexForChoice]));

    aiChoice = filteredPossibleChoices[indexForChoice];
